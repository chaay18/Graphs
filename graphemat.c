...
void parcoursLargeur (GrapheMat* graphe)
{
    ...
    for (int i=0; i<graphe->n; i++)
    {
        ...
        for(int j=0;j<graphe->n;j++){
            if ( (graphe->element [i*nMax+j] == vrai)&& !graphe->marque [j] )
                {
...
static void Largeur(GrapheMat* graphe, int numSommet){
    ...
    Objet* a = (Objet*) numSommet;
    insererEnFinDeListe(li,a);
    graphe->marque[numSommet] = vrai;
    while(!listeVide(li)){
        int a = (int)((size_t)extraireEnTeteDeListe(li));
//        printf("%d",);
        printf("%s",graphe->nomS[a]);
        for(int i=0; i<graphe->n; i++){
            if(graphe->element[a*nMax+i] && !graphe->marque[i]){
                Objet* iter = (Objet*) i;
                insererEnFinDeListe(li, iter);
                graphe->marque[i]=vrai;
            }
        }
    }
...
// non informe :DFS
void parcoursLargeur1(GrapheMat* graphe){
    razMarque(graphe);
    for(int i=0 ; i<graphe->n; i++) {
